...
constexpr const float kDefaultRolloff = -6.0f;
constexpr const float kMaxRolloff = -200.0f;
...
void SettingsPositionalAudio::Init(PositionalAudio *positionalAudio)
{
    if (m_ContextMenuUi == -1)
    {
        ...
        connect(&context_menu, &TSContextMenu::MenusInitialized, this,
                &SettingsPositionalAudio::onMenusInitialized, Qt::AutoConnection);
        connect(&context_menu, &TSContextMenu::FireContextMenuEvent, this,
                &SettingsPositionalAudio::onContextMenuEvent, Qt::AutoConnection);
    }
    ...
    connect(this, &SettingsPositionalAudio::UseAttenuationSet, positionalAudio,
            &PositionalAudio::setUseAttenuation);
    connect(this, &SettingsPositionalAudio::DistanceMinChanged, positionalAudio,
            &PositionalAudio::setDistanceMin);
    connect(this, &SettingsPositionalAudio::DistanceMaxChanged, positionalAudio,
            &PositionalAudio::setDistanceMax);
    connect(this, &SettingsPositionalAudio::RollOffChanged, positionalAudio, &PositionalAudio::setRollOff);
    connect(this, &SettingsPositionalAudio::RollOffMaxChanged, positionalAudio,
            &PositionalAudio::setRollOffMax);

    connect(this, &SettingsPositionalAudio::ServerSettingsAdd, positionalAudio,
            &PositionalAudio::AddServerSetting);
    connect(this, &SettingsPositionalAudio::ServerSettingsRemove, positionalAudio,
            &PositionalAudio::RemoveServerSetting);
    connect(this, &SettingsPositionalAudio::ServerEnabledSet, positionalAudio,
            &PositionalAudio::setServerSettingEnabled);
    connect(this, &SettingsPositionalAudio::ServerBlockSet, positionalAudio,
            &PositionalAudio::setServerSettingBlocked);
    connect(this, &SettingsPositionalAudio::ServerSendIntervalChange, positionalAudio,
            &PositionalAudio::setServerSettingSendInterval);
    connect(this, &SettingsPositionalAudio::ServerSendIntervalSilentIncChange, positionalAudio,
            &PositionalAudio::setServerSettingSendIntervalSilentInc);

    // pure display
    connect(positionalAudio, &PositionalAudio::myIdentityChanged, this,
            &SettingsPositionalAudio::UpdateUIStatusSelfName, Qt::QueuedConnection);
    connect(positionalAudio, &PositionalAudio::myVrChanged, this,
            &SettingsPositionalAudio::UpdateUIStatusSelfGame, Qt::QueuedConnection);
    ...
    else
    {
        for (int i = 0; i < groups.size(); ++i)
        {
            ...
            emit ServerSendIntervalSilentIncChange(sUId,
                                                   cfg.value("send_interval_silentinc", 1.0f).toFloat());
            cfg.endGroup();
        }
    }
    cfg.endGroup();
    ...
    emit RollOffChanged(cfg.value("rolloff", kDefaultRolloff).toFloat());
    emit RollOffMaxChanged(cfg.value("rolloff_max", kMaxRolloff).toFloat());
...
void SettingsPositionalAudio::onContextMenuEvent(uint64 serverConnectionHandlerID,
                                                 PluginMenuType type,
                                                 int menuItemID,
                                                 uint64 selectedItemID)
{
    ...
    if (type == PLUGIN_MENU_TYPE_GLOBAL)
    {
        if (menuItemID == m_ContextMenuUi)
        {
            ...
            {
                auto *p_config =
                new ConfigPositionalAudio(TSHelpers::GetMainWindow());  // has delete on close attribute

                QSettings cfg(TSHelpers::GetPath(teamspeak::plugin::Path::PluginIni), QSettings::IniFormat);
                cfg.beginGroup(mP_positionalAudio.data()->objectName());
                // p_config->UpdateUIEnabledSet(cfg.value("enabled",true).toBool());
                this->connect(this, &SettingsPositionalAudio::UpdateUIEnabledSet, p_config,
                              &ConfigPositionalAudio::UpdateUIEnabledSet);
                emit UpdateUIEnabledSet(cfg.value("enabled", true).toBool());
                this->connect(this, &SettingsPositionalAudio::UpdateUIUseCameraSet, p_config,
                              &ConfigPositionalAudio::UpdateUIUseCameraSet);
                emit UpdateUIUseCameraSet(cfg.value("isUseCamera", true).toBool());

                cfg.beginGroup("attenuation");
                this->connect(this, &SettingsPositionalAudio::UpdateUIUseAttenuationSet, p_config,
                              &ConfigPositionalAudio::UpdateUIUseAttenuationSet);
                emit UpdateUIUseAttenuationSet(cfg.value("enabled", false).toBool());
                this->connect(this, &SettingsPositionalAudio::UpdateUIDistanceMin, p_config,
                              &ConfigPositionalAudio::UpdateUIDistanceMin);
                emit UpdateUIDistanceMin(cfg.value("distance_min", 0).toInt());
                this->connect(this, &SettingsPositionalAudio::UpdateUIDistanceMax, p_config,
                              &ConfigPositionalAudio::UpdateUIDistanceMax);
                emit UpdateUIDistanceMax(cfg.value("distance_max", 0).toInt());
                this->connect(this, &SettingsPositionalAudio::UpdateUIRollOff, p_config,
                              &ConfigPositionalAudio::UpdateUIRollOff);
                emit UpdateUIRollOff(cfg.value("rolloff", kDefaultRolloff).toFloat());
                this->connect(this, &SettingsPositionalAudio::UpdateUIRollOffMax, p_config,
                              &ConfigPositionalAudio::UpdateUIRollOffMax);
                emit UpdateUIRollOffMax(cfg.value("rolloff_max", kMaxRolloff).toFloat());
                cfg.endGroup();

                this->connect(this, &SettingsPositionalAudio::UpdateUIServerSettingsAdd, p_config,
                              &ConfigPositionalAudio::UpdateUIServerAdd);
                this->connect(this, &SettingsPositionalAudio::UpdateUIServerSettingsRemove, p_config,
                              &ConfigPositionalAudio::UpdateUIServerRemove);
                this->connect(this, &SettingsPositionalAudio::UpdateUIServerEnabled, p_config,
                              &ConfigPositionalAudio::UpdateUIServerEnabled);
                this->connect(this, &SettingsPositionalAudio::UpdateUIServerBlock, p_config,
                              &ConfigPositionalAudio::UpdateUIServerBlocked);
                this->connect(this, &SettingsPositionalAudio::UpdateUIServerSendInterval, p_config,
                              &ConfigPositionalAudio::UpdateUIServerSendInterval);
                this->connect(this, &SettingsPositionalAudio::UpdateUIServerSendIntervalSilentInc, p_config,
                              &ConfigPositionalAudio::UpdateUIServerSendIntervalSilentInc);
                this->connect(this, &SettingsPositionalAudio::UpdateUIServerSelect, p_config,
                              &ConfigPositionalAudio::UpdateUIServerSelect);
                ...
                else
                {
                    ...
                    for (int i = 0; i < groups.size(); ++i)
                    {
                        ...
                        emit UpdateUIServerSendIntervalSilentInc(
                        sUId, cfg.value("send_interval_silentinc", 1.0f).toFloat());
                        cfg.endGroup();
                    }
                }
                ...
                this->connect(p_config, &ConfigPositionalAudio::EnabledSet, this,
                              &SettingsPositionalAudio::EnabledSet);
                this->connect(p_config, &ConfigPositionalAudio::CameraSet, this,
                              &SettingsPositionalAudio::UseCameraSet);
                this->connect(p_config, &ConfigPositionalAudio::AttenuationSet, this,
                              &SettingsPositionalAudio::UseAttenuationSet);
                this->connect(p_config, &ConfigPositionalAudio::DistanceMinChanged, this,
                              &SettingsPositionalAudio::DistanceMinChanged);
                this->connect(p_config, &ConfigPositionalAudio::DistanceMaxChanged, this,
                              &SettingsPositionalAudio::DistanceMaxChanged);
                this->connect(p_config, &ConfigPositionalAudio::RollOffChanged, this,
                              &SettingsPositionalAudio::RollOffChanged);
                this->connect(p_config, &ConfigPositionalAudio::RollOffMaxChanged, this,
                              &SettingsPositionalAudio::RollOffMaxChanged);

                this->connect(p_config, &ConfigPositionalAudio::ServerEnabledSet, this,
                              &SettingsPositionalAudio::ServerEnabledSet);
                this->connect(p_config, &ConfigPositionalAudio::ServerSendIntervalChange, this,
                              &SettingsPositionalAudio::ServerSendIntervalChange);
                this->connect(p_config, &ConfigPositionalAudio::ServerSendIntervalSilentIncChange, this,
                              &SettingsPositionalAudio::ServerSendIntervalSilentIncChange);
                this->connect(p_config, &ConfigPositionalAudio::ServerBlockSet, this,
                              &SettingsPositionalAudio::ServerBlockSet);

                this->connect(p_config, &ConfigPositionalAudio::ServerAddButtonClicked, this,
                              &SettingsPositionalAudio::AddCustomServerSettingForCurrentTab);
                this->connect(p_config, &ConfigPositionalAudio::ServerRemoveButtonClicked, this,
                              &SettingsPositionalAudio::RemoveCustomServerSettingByServerUniqueId);

                connect(p_config, &ConfigPositionalAudio::finished, this,
                        &SettingsPositionalAudio::saveSettings);

                // pure display
                this->connect(this, &SettingsPositionalAudio::UpdateUIStatusSelfName, p_config,
                              &ConfigPositionalAudio::UpdateUIStatusSelfName);
                emit UpdateUIStatusSelfName(
                QString::fromStdWString(mP_positionalAudio.data()->getMyIdentity()));
                this->connect(this, &SettingsPositionalAudio::UpdateUIStatusSelfGame, p_config,
                              &ConfigPositionalAudio::UpdateUIStatusSelfGame);
                emit UpdateUIStatusSelfGame(QString::fromStdWString(mP_positionalAudio.data()->get_my_vr()));
...
{
    const auto connection_id = pluginsdk::funcs::get_current_server_connection_handler_id();
    if (connection_id != (uint64) NULL)
        AddCustomServerSetting(connection_id);
}
...
void SettingsPositionalAudio::AddCustomServerSetting(uint64 serverConnectionHandlerID)
{
    ...
    if (!server_info)
    {
        ...
        msgBoxNotConnected.setText(
        "You need to be connected to the server you want to add a custom setting for.");
    ...
    if (config && config.data()->isVisible())
    {
        ...
        //    msgBox->setAttribute( Qt::WA_DeleteOnClose, true ); //makes sure the msgbox is deleted
...
void SettingsPositionalAudio::SetServerBlock(QString serverUniqueId,
                                             bool val,
                                             uint64 serverConnectionHandlerID)
{
    ...
    else if (val)  // add a custom server setting
    {
        if (serverConnectionHandlerID)
        {
            ...
            TSLogging::Error("(SetServerBlock) Blocking a new server without serverConnectionHandlerID is "
                             "not implemented yet.");
    }
    ...
    if (val)
    {
        ...
        auto *server_info = m_servers_info.get_server_info(serverConnectionHandlerID);
        msg_box->setText(QString("Flood Protection triggered for server %1.")
                         .arg(server_info ? server_info->getName() : "unknown"));
        msg_box->setInformativeText(
        tr("The server has been blocked for Positional Audio updates.\n To change your send intervals and "
           "unblock it go to the config window.\n Do so now?"));
        ...
        auto *layout = dynamic_cast<QGridLayout *>(msg_box->layout());
...
void SettingsPositionalAudio::saveSettings(int r)
{
    ...
    QSettings cfg(TSHelpers::GetPath(teamspeak::plugin::Path::PluginIni), QSettings::IniFormat);
    cfg.beginGroup(mP_positionalAudio.data()->objectName());
    cfg.setValue("enabled", mP_positionalAudio.data()->enabled());
...
void SettingsPositionalAudio::serverBlockMsgBoxClosed(QAbstractButton *button)
{
    if (mP_serverBlockMsgBox)
    {
        if (QMessageBox::YesRole == mP_serverBlockMsgBox.data()->buttonRole(button))
        {
            ...
            emit UpdateUIServerSelect(
            mP_serverBlockMsgBox.data()->property("TargetServerUniqueId").toString());
        }
    }
